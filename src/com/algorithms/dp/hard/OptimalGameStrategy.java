...
public class OptimalGameStrategy {
	...
	public int findPlayer1MaxGains(int[] coins){
		...
		for(i = 0; i < totalCoins; i++){
			scores[i][i] = new Game(coins[i], 0);			//Initializing scores matrix considering there is only element in coins array. So obviously player 1 selects it and player 2 gets 0. 
		}
		
		/**
		 * Var k is to go diagonally in the matrix
		 */
		for(k = 2; k <= totalCoins; k++){
			for(i = 0; i <= totalCoins - k; i++){
				 j = i + k -1;
				 /**
				  * Calculating score when player1 has chosen the left end coin
				  */
				 int totalWithLeftEndCoin = coins[i] + scores[i + 1][j].player2Score;
				 
				 /**
				  * Calculating score when player1 has chosen the right end coin
				  */
